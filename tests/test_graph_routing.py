...
def test_default_file_generator_uses_bedrock_dynamic_generator(monkeypatch):
    calls = []
    monkeypatch.delenv("IAC_SMITH_GENERATION_MODE", raising=False)
...
def test_default_file_generator_uses_spec_renderer_when_enabled(monkeypatch):
    calls = []

    class FakeSpecRendererGenerator:
        def generate_files(self, **kwargs):
            calls.append(kwargs)
            return {"modules/example/main.tf": "# rendered from spec\n"}

    monkeypatch.setenv("IAC_SMITH_GENERATION_MODE", "spec_renderer")
    monkeypatch.setattr("iac_smith.graph.SpecRendererGenerator", FakeSpecRendererGenerator)
    intent = InfrastructureIntent(
        raw_request="Create example infrastructure",
        resource_type="example",
        environment_scope=EnvironmentScope.NON_PROD_ONLY,
    ...
    assert files == {"modules/example/main.tf": "# rendered from spec\n"}
