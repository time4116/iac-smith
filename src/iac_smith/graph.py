...
def intent_parser(state: IaCSmithState) -> IaCSmithState:
    intent = parse_intent(state["issue_body"])
    if intent.blocked:
        return {
            **state,
            "intent": intent,
...
def change_planner(state: IaCSmithState) -> IaCSmithState:
    return {
        **state,
        "change_plan": plan_changes(state["intent"], state["target_repo"]),
        "status": "planned",
    }
...
def build_graph():
    ...
    graph.add_edge("ruleset_loader", "change_planner")
    graph.add_edge("change_planner", "validation_runner")
    graph.add_conditional_edges(
        "validation_runner",
