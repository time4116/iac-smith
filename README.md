...
Module bodies are filled by the spec composer: the model proposes typed resource selections (resource types, arguments, nested blocks) as JSON, never HCL text, and every proposal is validated against the harvested provider schema — hallucinated types, unsupported arguments or blocks, missing required arguments, and references to undeclared variables or non-existent sibling resources are rejected and repaired at the JSON level before anything is rendered. Composition is generic: the candidate universe is whatever `terraform providers schema -json` exposes for the providers the generated files declare. When composition is unavailable (no `BEDROCK_MODEL_ID`, `IAC_SMITH_SPEC_COMPOSER=0`, or schema harvest failure) the renderer degrades to deterministic structure-only output, and generated PR bodies explicitly warn when a PR is structure-only and contains placeholder module bodies.
