...
3. `AWS_ROLE_ARN_NON_PROD`

   GitHub Actions secret containing the non-production AWS OIDC role ARN used for Bedrock access and non-prod validation workflows.

4. `AWS_ROLE_ARN_PROD`

   GitHub Actions secret containing the production AWS OIDC role ARN. Required before generating or validating production environment workflows.
